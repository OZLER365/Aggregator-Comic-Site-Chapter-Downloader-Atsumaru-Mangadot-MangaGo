// ==UserScript==
// @name         Aggregator Comic Site Chapter Downloader(Atsumaru, Mangadot, MangaGo)
// @namespace    http://tampermonkey.net/
// @version      1.0.1
// @description  Downloads chapter images, Native ZIP, SPA Ready. Combined script for Atsu, Mangadot, and MangaGo.
// @author       ozler365
// @icon         https://atsu.moe//favicon/apple-touch-icon.png
// @license      MIT
// @match        https://atsu.moe/*
// @match        https://mangadot.net/*
// @match        *://*.mangago.me/*
// @match        *://*.mangago.zone/*
...
// @downloadURL https://update.greasyfork.org/scripts/595572/Aggregator%20Comic%20Site%20Chapter%20Downloader%28Atsumaru%2C%20Mangadot%2C%20MangaGo%29.user.js
// @updateURL https://update.greasyfork.org/scripts/595572/Aggregator%20Comic%20Site%20Chapter%20Downloader%28Atsumaru%2C%20Mangadot%2C%20MangaGo%29.meta.js
// ==/UserScript==
...
(function() {
    ...
    function runAtsuScript() {
        ...
        function injectButton() {
            // Find the robust desktop container path from your screenshot
            const desktopContainer = document.querySelector('div.absolute.top-12.right-12.z-10 > div.flex-col') || document.querySelector('div.absolute.top-12.right-12.z-10.h-fit > div.flex.flex-col');
            const isMobile = !desktopContainer;
            let btn = document.getElementById('atsu-dl-btn');

            // Handle SPA layout shifts: if a button exists but in the wrong container, destroy it so it regenerates properly
            if (btn) {
            ...
            if (isMobile) {
                ...
                // Exact styling classes based on your screenshot's native elements
    ...
    function runMangaGoScript() {
        ...
        async function startDownload() {
            ...
            filesData = filesData.filter(Boolean);
            btn.innerHTML = !nextBtn ? `<span style="font-size: 11px; font-weight: bold;">ZIP</span>` : `Zipping...`;
            ...
            try {
                ...
                GM_download({
                    ...
                    saveAs: false,
                    onload: () => {
                        if(!nextBtn) btn.innerHTML = `<span style="font-size: 11px; font-weight: bold;">Done</span>`;
                    ...
                    onerror: (err) => {
                        console.error("GM_download failed:", err);
                        if(!nextBtn) btn.innerHTML = `<span style="font-size: 11px; font-weight: bold;">Err</span>`;
